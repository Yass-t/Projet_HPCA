...
void PDE_diff (float dt, float dx, float dsig, float pmin, float pmax, 
	...
	*/
	...
	//PDE_diff_k2 <<<NB, NTPB, sizeof(float)*NTPB >>> (dt, dx, dsig, pmin, pmax, sigmin, N, GPUTab);
	PDE_diff_k3 <<<NB, NTPB, sizeof(float)*NTPB >>> (dt, dx, dsig, pmin, pmax, sigmin, N, GPUTab);
	//PDE_diff_k4 <<<NB, NTPB, sizeof(float)*NTPB >>> (dt, dx, dsig, pmin, pmax, sigmin, N, GPUTab);
